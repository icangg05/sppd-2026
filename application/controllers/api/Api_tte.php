...
class Api_tte extends qr_Controller {
	...
	function cetak_spd($jenis_sppd, $posisi, $telaah_id, $pengikut_id, $nik, $posisi_penandatangan)
	{
		...
		$pdf->Cell(80,4,$data[0]['pangkat']." - ".$data[0]['pegawai_golongan'],'TR',1);
	...
	function cetak_spt($telaah_id, $posisi, $kategori_pelaksana, $pegawai_id, $nik, $posisi_penandatangan)
	{
		...
		if(($posisi=="2" && $kategori_pelaksana==2 && $data[0]['telaah_domainperjalanan']==1) 
	...
	function cetak_spt_dprd($telaah_id,$kategori_pelaksana,$pegawai_id,$nik, $posisi_penandatangan)
	{
        ...
        $pdf->SetFont('Times','',10);
        $pdf->Cell(45,5,'',0,0);
        $pdf->Cell(115,5,'NOMOR :',0,1);
		
        $pdf->Cell(10,4,'',0,1);
		
        $pdf->SetFont('Times','',10);
        $pdf->Cell(5,5,'1.',0,0);
        $pdf->Cell(15,5,'Dasar',0,0);
        $pdf->Cell(140,5,':',0,1);
		
        $pdf->Cell(5,5,'',0,0);
        $pdf->Cell(5,5,'a.',0,0);
        $pdf->Cell(150,5,'Perda Kota Kendari no. 5 Tahun 2022 tentang APBD Kota Kendari tahun '.date('Y').';',0,1);
		
        $pdf->Cell(5,5,'',0,0);
        $pdf->Cell(5,5,'b.',0,0);
        $pdf->Cell(150,5,'Peraturan Tata Tertib Dewan Perwakilan Rakyat Daerah Kota Kendari;',0,1);
		
        $pdf->Cell(5,5,'',0,0);
        $pdf->Cell(5,5,'c.',0,0);
        $pdf->Cell(150,5,'Program Kerja DPRD Kota Kendari tahun '.date('Y').';',0,1);
		
        $pdf->Cell(5,5,'',0,0);
        $pdf->Cell(5,5,'d.',0,0);
        $pdf->Cell(150,5,'DPA-SKPD Sekretariat DPRD Kota Kendari;',0,1);
		
        $pdf->Cell(5,5,'',0,0);
        $pdf->Cell(155,5,'Menugaskan Kepada Anggota DPRD Kota Kendari yang tercantum namanya dibawah ini',0,1);
		
        $pdf->Cell(5,5,'',0,1);
		
		$pdf->SetFont('Times','B',10);
        $pdf->Cell(5,5,'No',1,0,'C');
        $pdf->Cell(75,5,'N a m a',1,0,'C');
        $pdf->Cell(70,5,'Jabatan',1,0,'C');
        $pdf->Cell(10,5,'Ket',1,1,'C');
		
		$pdf->SetFont('Times','',9);
        $pdf->Cell(5,5,'1.',1,0,'C');
        $pdf->Cell(75,5,$data[0]['anggotadprd_name'],1,0,'C');
        $pdf->Cell(70,5,$data[0]['anggotadprd_jabatan'],1,0,'C');
        $pdf->Cell(10,5,'',1,1,'C');
		...
		foreach($data2 as $v) {
			$pdf->Cell(5,5,$no++.'.',1,0,'C');
			$pdf->Cell(75,5,$v->anggotadprd_name,1,0,'C');
			$pdf->Cell(70,5,$v->anggotadprd_jabatan,1,0,'C');
			$pdf->Cell(10,5,'',1,1,'C');
		}
		...
		$pdf->SetFont('Times','',10);
        $pdf->Cell(5,5,'2.',0,0);
        $pdf->Cell(50,5,'Tujuan Perintah Tugas ',0,0);
        $pdf->Cell(10,5,':',0,1);
		
        $pdf->Cell(5,5,'',0,0);
        $pdf->MultiCell(165,5, $data[0]['telaah_perihal'],0,'J');
        
		$pdf->Cell(10,4,'',0,1);
		
        $pdf->Cell(5,5,'3.',0,0);
        $pdf->Cell(50,5,'Waktu dan Tempat Pelaksanaan',0,0);
        $pdf->Cell(10,5,':',0,1);
        ...
        $pdf->MultiCell(100,5,$dayList[$day1].', '.$this->tgl_indo($data[0]['telaah_tanggalberangkat']).' s/d '.$dayList[$day2].', '. $this->tgl_indo($data[0]['telaah_tanggalkembali']).'.',0,'J');
		
		$pdf->Cell(5,5,'',0,0);
        $pdf->Cell(5,5,'b.',0,0);
        $pdf->Cell(25,5,'Tempat',0,0);
        $pdf->Cell(5,5,':',0,0);
        $pdf->MultiCell(100,5,$data[0]['telaah_kantortujuan'],0,'J');
		
		$pdf->Cell(10,4,'',0,1);
		
        $pdf->MultiCell(160,5,'Demikian Surat Perintah Tugas ini diberikan untuk dilaksanakan dengan penuh rasa tanggung jawab dan apabila Surat Perintah Tugas Ini tidak dijalankan sesuai aturan Perundang-Undangan yang berlaku, maka yang bersangkutan dan/atau penerima Surat Perintah Tugas ini yang akan bertanggung jawab.',0,'J');
		...
		$pdf->SetFont('Times','',10);
        $pdf->Cell(20,5,'Tembusan Yth:',0,0);
        $pdf->Cell(3,5,':',0,0);
        $pdf->Cell(137,5,'',0,1);
        ...
        $pdf->Cell(20,5,'',0,0);
        $pdf->Cell(3,5,'',0,0);
		$pdf->Cell(137,5,'1. Walikota kendari di Kendari',0,1);
		
		$pdf->SetFont('Times','',10);
        $pdf->Cell(20,5,'',0,0);
        $pdf->Cell(3,5,'',0,0);
        $pdf->Cell(137,5,'2. Arsip',0,1);
	...
	function generate_tte ($user_id, $telaah_id)
	{
		...
		$telaah_idx = base64_encode($this->encrypt->encode($telaah_id, $this->session->userdata('encrypt_key')));
		
		if($user[0]['group_id']==4 && $user[0]['jenis_skpd']!=2){
			$posisi = "kadis";
			...
			$posisi = "sekda";
		} else if($user[0]['group_id']==13){
			$posisi = "lurah";
		}
		
		switch($posisi){
			case "kadis" 		: 	$pegawai_nik= $this->m_spd->kepala_opd($user[0]['skpd_id']);break;
			...
			case "kapus" 		: 	$pegawai_nik= $this->m_spd->kapus($user[0]['skpd_id']);break;
			case "camat" 		: 	$pegawai_nik= $this->m_spd->camat($user[0]['skpd_id']);break;
			case "sekda" 		: 	$pegawai_nik= $this->m_spd->sekda();break;
			case "lurah" 		: 	$pegawai_nik= $this->m_spd->lurah($user[0]['skpd_id']);break;
			case "walikota2" 	: 	if($user[0]['group_id'] == 6){
		...
		if($nik==""){
			$resultData[] = array(	'status'=> "0",
									'message'=> 'NIK Pegawai Belum Diinput !!!'
									);
			echo json_encode($resultData, JSON_PRETTY_PRINT);
		} else {
			
			if($telaah[0]['telaah_kategori']==3){
				$data =  $this->m_telaah->get_dprd($telaah_id);
			} else {
				if($telaah[0]['telaah_kategori']==8){
					$data =  $this->m_telaah->getWalikota($telaah_id);
				} else {
			...
			if($status_nik['status_code']==1111){
				...
				if($telaah[0]['telaah_kategori']==3){
					...
					## SPT DPRD (3)
					if($telaah[0]['telaah_kategori']==3 && $posisi=='kadprd'){
						$this->cetak_spt_dprd($telaah_id,1,$data[0]['anggotadprd_id'],$nik,$posisi);
					} 
					
					if($telaah[0]['telaah_kategori']==3 && $posisi=='sekwan'){
						$this->cetak_spd('dprd',1,$telaah_id,'',$nik,$posisi);
					} 
					
					$data2 =  $this->m_telaah->get_dprd($telaah_id);
					...
					## SPT Kepala OPD (2)
					if($telaah[0]['telaah_kategori']==2 && $posisi=='walikota'){
						$this->cetak_spt($telaah_id,$telaah[0]['telaah_kategori'],1, $data[0]['pegawai_id'],$nik,$posisi);
						
					} else if($telaah[0]['telaah_kategori']==2 && $posisi=='kadis'){
						$this->cetak_spd('opd',1,$telaah_id,'',$nik,$posisi);
						$this->cetak_spt($telaah_id,$telaah[0]['telaah_kategori'],1, $data[0]['pegawai_id'],$nik,$posisi);
					## SPT SEKDA (4)
					} else if($telaah[0]['telaah_kategori']==4 && $posisi=='walikota'){
						$this->cetak_spt($telaah_id,$telaah[0]['telaah_kategori'],1, $data[0]['pegawai_id'],$nik,$posisi);
					## LURAH DAN CAMAT (5)
					} else if($telaah[0]['telaah_kategori']==5 && $posisi=='sekda'){
						$this->cetak_spt($telaah_id,$telaah[0]['telaah_kategori'],1, $data[0]['pegawai_id'],$nik,$posisi);
					} else if($telaah[0]['telaah_kategori']==5 && $posisi=='camat'){
						$this->cetak_spd('opd',1,$telaah_id,'',$nik,$posisi);
					
					## WALIKOTA (8)
					} else if($telaah[0]['telaah_kategori']==8 && $user[0]['group_id'] == 8){
						$this->cetak_spt($telaah_id,$telaah[0]['telaah_kategori'],1, $data[0]['pegawai_id'],$nik,$posisi);
					} else if($telaah[0]['telaah_kategori']==8 && $user[0]['group_id'] == 6){
						$this->cetak_spd('walikota',1,$telaah_id,'',$nik,$posisi);		
					## SPT SEKWAN (10)
					} else if($telaah[0]['telaah_kategori']==10 && $posisi=='walikota'){
						$this->cetak_spt($telaah_id,$telaah[0]['telaah_kategori'],1, $data[0]['pegawai_id'],$nik,$posisi);
					## 1,6,7,9,11
					} else {
						$this->cetak_spd('opd',1,$telaah_id,'',$nik,$posisi);
					...
					if($telaah[0]['telaah_kategori']==8){
						$data2 =  $this->m_telaah->getWalikota($telaah_id);
					} else {
						$data2 =  $this->m_telaah->get($telaah_id);
					}
				...
				if($telaah[0]['telaah_kategori']==3){
					$data2 =  $this->m_pengikut->data_dprd($telaah_id);
				} else {
					$data2 =  $this->m_pengikut->get_pengikut($telaah_id);
				}
						...
						foreach($data2 as $v){
							...
							if($telaah[0]['telaah_kategori']==3){
								$this->cetak_spd('dprd',2,$telaah_id,$v->anggotadprd_id,$nik,$posisi);
							} else if($telaah[0]['telaah_kategori']==8 && $user[0]['group_id'] == 6){
								$this->cetak_spd('walikota',2,$telaah_id,$v->pegawai_id,$nik,$posisi);
							} else if($telaah[0]['telaah_kategori']==8 && $user[0]['group_id'] == 8){
							} else {
								$this->cetak_spd('opd',2,$telaah_id,$v->pegawai_id,$nik,$posisi);
							}
							
							$koordinat_tte_pengikut = $this->m_pengikut->get_koordinat_tte_pengikut($telaah_id, $v->pegawai_id);
							...
							## Tidak pakai SPT
							if(($telaah[0]['telaah_kategori']==2 && $posisi=='walikota') 
								|| ($telaah[0]['telaah_kategori']==3 && $posisi=='kadprd') 
								|| ($telaah[0]['telaah_kategori']==4 && $posisi=='walikota') 
								|| ($telaah[0]['telaah_kategori']==5 && $posisi=='sekda')
								|| ($telaah[0]['telaah_kategori']==8 && $user[0]['group_id'] == 8) 
								|| ($telaah[0]['telaah_kategori']==10 && $posisi=='walikota')  ){
							...
							## 1,6,7,9,11
								...
								$send_sign_request   = $this->sendSignRequest($filename2,$set_y2,$set_height2,$nik, $this->input->post('passphrase'),$telaah_idx  );
								$send_sign_request   = json_decode($send_sign_request , TRUE);
								
								$data6['telaah_id'] = $telaah_id;
								...
								// ## Halaman Kedua SPPD
								$send_sign_request4   = $this->sendSignRequest4($filename_tte_sppd,$nik,$this->input->post('passphrase'),$telaah_idx  );
				...
				$get_halaman = $this->m_telaah->get($telaah_id);
				...
				if(($telaah[0]['telaah_kategori']==2 && $posisi=='walikota')
					...
					if($telaah[0]['telaah_kategori']==3){
						$send_sign_request3   = $this->sendSignRequest3($filename3,$get_halaman[0]['telaah_halaman_tte'],$set_y3,$set_height3,$nik, $this->input->post('passphrase'),$telaah_idx  );
						$send_sign_request3   = json_decode($send_sign_request3 , TRUE);
							
						// if($send_sign_request3['id_signed']==""){
							// $this->cetak_spt_dprd2($telaah_id,1,$data[0]['anggotadprd_id'],$nik,$posisi);
						// }else {
						...
						$send_sign_request3   = $this->sendSignRequest2($filename3,$get_halaman[0]['telaah_halaman_tte'],$set_y3,$set_height3,$nik, $this->input->post('passphrase'),$telaah_idx  );
							$send_sign_request3   = json_decode($send_sign_request3 , TRUE);
				...
				## Hanya SPD
				} else if(($telaah[0]['telaah_kategori']==3 && $posisi=='sekwan')
						||($telaah[0]['telaah_kategori']==4 && $posisi=='sekda')
						||($telaah[0]['telaah_kategori']==5 && $posisi=='camat')
						||($telaah[0]['telaah_kategori']==8 && $user[0]['group_id'] == 6)
						||($telaah[0]['telaah_kategori']==10 && $posisi=='sekwan')){
					...
					$send_sign_request   = $this->sendSignRequest($filename,$set_y,$set_height,$nik, $this->input->post('passphrase'),$telaah_idx  );
					$send_sign_request   = json_decode($send_sign_request , TRUE);
					
					// if($send_sign_request['id_signed']==""){
						// if($telaah[0]['telaah_kategori']==3){
							// $this->cetak_spd2('dprd',1,$telaah_id,'',$nik,$posisi);
						// } else if($telaah[0]['telaah_kategori']==8){
							// $this->cetak_spd2('walikota',1,$telaah_id,'',$nik,$posisi);
						// } else {
							// $this->cetak_spd2('opd',1,$telaah_id,'',$nik,$posisi);
						// }
					// } else {
						$download_document  = $this->downloadDokumenById($send_sign_request['id_dokumen'],$filename_tte_sppd);
						$download_document  = json_decode($download_document , TRUE);
				...
				if($send_sign_request['error']){
					...
					$log_tte['telaah_id'] = $telaah_id;
					$log_tte['id_user'] = $user_id;
					$log_tte['pegawai_id'] = $data[0]['pegawai_id'];
					$log_tte['action'] = $send_sign_request['error'];
					$log_tte['action_table'] = "Tracking SPPD";
					$this->m_log->create_tte2($log_tte);
					
					$resultData[] = array('status'=> "0",
											'message'=> $send_sign_request['error']
											);
					echo json_encode($resultData, JSON_PRETTY_PRINT);
					...
					$resultData[] = array('status'=> "0",
											'message'=> $send_sign_request2['error']
											);
					echo json_encode($resultData, JSON_PRETTY_PRINT);
					...
					$resultData[] = array('status'=> "0",
											'message'=> $send_sign_request3['error']
											);
					echo json_encode($resultData, JSON_PRETTY_PRINT);
					...
					$resultData[] = array('status'=> "0",
											'message'=> $send_sign_request4['error']
											);
					echo json_encode($resultData, JSON_PRETTY_PRINT);
					  
				} else {
					## Esselon
					if($telaah[0]['telaah_kategori']==1 && $posisi=='kadis'){
						$data3['telaah_id'] = $telaah_id;
						$data3['timeline_kadis_id'] = 1; 
						$data3['timeline_kadis_date'] = date("Y-m-d H:i:s");
						$data3['timeline_kadis_disposisi'] = "ACC";
						$this->m_api->update_timeline_1($data3,'');
						
						$data4['telaah_id'] = $telaah_id;
						$data4['telaah_status'] = 2;
						$this->m_api->update_timeline_1($data3, $data4);
					...
					if($telaah[0]['telaah_kategori']==2 && $posisi=='kadis'){
						$data3['telaah_id'] = $telaah_id;
						$data3['timeline_kadis_id'] = 1; 
						$data3['timeline_kadis_date'] = date("Y-m-d H:i:s");
						$data3['timeline_kadis_disposisi'] = "ACC";
						$this->m_api->update_timeline_2($data3,'');
						
						if ($data[0]['telaah_domainperjalanan']==3){
							$data4['telaah_id'] = $telaah_id;
							$data4['telaah_status'] = 2;
							$this->m_api->update_timeline_2($data3, $data4);
						}
					...
					if($telaah[0]['telaah_kategori']==3 && $posisi=='sekwan'){
						...
						$this->m_api->update_timeline_3($data3,'');
					...
					if($telaah[0]['telaah_kategori']==3 && $posisi=='kadprd'){
						...
						$data4['telaah_id'] = $telaah_id;
						$data4['telaah_status'] = 2;
						$this->m_api->update_timeline_3($data3, $data4);
					...
					if($telaah[0]['telaah_kategori']==4 && $posisi=='walikota'){
						$data3['telaah_id'] = $telaah_id;
						$data3['timeline_walikota_id'] = 1; 
						$data3['timeline_walikota_date'] = date("Y-m-d H:i:s");
						$data3['timeline_walikota_disposisi'] = "ACC";
						...
						$data4['telaah_id'] = $telaah_id;
						$data4['telaah_status'] = 2;
						$this->m_api->update_timeline_4($data3, $data4);
					...
					if($telaah[0]['telaah_kategori']==5 && $posisi=='sekda'){
						if($telaah[0]['jenis_skpd']==5){
							$data3['telaah_id'] = $telaah_id;
							$data3['timeline_sekda_id'] = 1; 
							$data3['timeline_sekda_date'] = date("Y-m-d H:i:s");
							$data3['timeline_sekda_disposisi'] = "ACC";
							...
							$data4['telaah_id'] = $telaah_id;
							$data4['telaah_status'] = 2;
							$this->m_api->update_timeline_5($data3, $data4);
					...
					if($telaah[0]['telaah_kategori']==6 && $posisi=='sekwan'){
						$data3['telaah_id'] = $telaah_id;
						$data3['timeline_sekwan_id'] = 1; 
						$data3['timeline_sekwan_date'] = date("Y-m-d H:i:s");
						$data3['timeline_sekwan_disposisi'] = "ACC";
						$this->m_api->update_timeline_6($data3,'');
						
						$data4['telaah_id'] = $telaah_id;
						$data4['telaah_status'] = 2;
						$this->m_api->update_timeline_6($data3, $data4);
					...
					if($telaah[0]['telaah_kategori']==7 && $posisi=='camat'){
						$data3['telaah_id'] = $telaah_id;
						$data3['timeline_camat_id'] = 1; 
						$data3['timeline_camat_date'] = date("Y-m-d H:i:s");
						$data3['timeline_camat_disposisi'] = 'ACC';
						...
						$data4['telaah_id'] = $telaah_id;
						$data4['telaah_status'] = 2;
						$this->m_api->update_timeline_7($data3, $data4);
					...
					if($telaah[0]['telaah_kategori']==8 && $user[0]['group_id'] == 6){
						$data3['telaah_id'] = $telaah_id;
						$data3['timeline_sekda_id'] = 1; 
						$data3['timeline_sekda_date'] = date("Y-m-d H:i:s");
						$data3['timeline_sekda_disposisi'] = 'ACC';
						$this->m_api->update_timeline_8($data3,'');
						...
						$log['action_table'] = "Tracking SPPD";
						$this->m_log->create2($log);
					} if($telaah[0]['telaah_kategori']==8 && $user[0]['group_id'] == 8){
						$data3['telaah_id'] = $telaah_id;
						$data3['timeline_walikota_id'] = 1; 
						$data3['timeline_walikota_date'] = date("Y-m-d H:i:s");
						$data3['timeline_walikota_disposisi'] = 'ACC';
						...
						$data4['telaah_id'] = $telaah_id;
						$data4['telaah_status'] = 2;
						$this->m_api->update_timeline_8($data3, $data4);
					...
					if($telaah[0]['telaah_kategori']==9 && $posisi=='sekda'){
						$data3['telaah_id'] = $telaah_id;
						$data3['timeline_sekda_id'] = 1; 
						$data3['timeline_sekda_date'] = date("Y-m-d H:i:s");
						$data3['timeline_sekda_disposisi'] = 'ACC';
						$this->m_api->update_timeline_9($data3,'');
						
						$data4['telaah_id'] = $telaah_id;
						$data4['telaah_status'] = 2;
						$this->m_api->update_timeline_9($data3, $data4);
					...
					if($telaah[0]['telaah_kategori']==10 && $posisi=='sekwan'){
						$data3['telaah_id'] = $telaah_id;
						$data3['timeline_sekwan_id'] = 1; 
						$data3['timeline_sekwan_date'] = date("Y-m-d H:i:s");
						$data3['timeline_sekwan_disposisi'] = 'ACC';
						$this->m_api->update_timeline_10($data3,'');
						...
						$log['action_table'] = "Tracking SPPD";
						$this->m_log->create2($log);
					}if($telaah[0]['telaah_kategori']==10 && $posisi=='walikota'){
						$data3['telaah_id'] = $telaah_id;
						$data3['timeline_walikota_id'] = 1; 
						$data3['timeline_walikota_date'] = date("Y-m-d H:i:s");
						$data3['timeline_walikota_disposisi'] = 'ACC';
						...
						$data4['telaah_id'] = $telaah_id;
						$data4['telaah_status'] = 2;
						$this->m_api->update_timeline_10($data3, $data4);
					...
					if($telaah[0]['telaah_kategori']==11 && $posisi=='kapus'){
						$data3['telaah_id'] = $telaah_id;
						$data3['timeline_kapus_id'] = 1; 
						$data3['timeline_kapus_date'] = date("Y-m-d H:i:s");
						$data3['timeline_kapus_disposisi'] = 'ACC';
						$this->m_api->update_timeline_11($data3,'');
						
						$data4['telaah_id'] = $telaah_id;
						$data4['telaah_status'] = 2;
						$this->m_api->update_timeline_11($data3, $data4);
					...
					// if($send_sign_request['id_signed']=="" && $send_sign_request3['id_signed']==""){
						...
						// $data5['telaah_id'] = $telaah_id;
						// if($telaah[0]['telaah_kategori']==1 || $telaah[0]['telaah_kategori']==6 
							// || $telaah[0]['telaah_kategori']==7 
							// || $telaah[0]['telaah_kategori']==9 || $telaah[0]['telaah_kategori']==11){
						...
						// $resultData[] = array('status'=> "2",
											// 'message'=> 'Server BSRE Tidak Merespon !! SPPD Menggunakan Tanda Tangan Manual'
											// );
						// echo json_encode($resultData, JSON_PRETTY_PRINT);
					...
					// } else {
						...
						$resultData[] = array('status'=> "1",
											'message'=> 'Dokumen TTE Berhasil Dibuat',
											'filename spd'=> $filename_tte_sppd,
											'filename spt'=> $filename_tte_spt
											);
						echo json_encode($resultData, JSON_PRETTY_PRINT);
					// }
	...
	function sendSignRequest($filename, $y, $height, $nik, $passphrase,$telaah_idx )
	{
		$curl 	= curl_init();
		...
		curl_setopt_array($curl, array(
    		CURLOPT_URL => "http://103.85.5.99/api/sign/pdf?nik=".$nik."&passphrase=".$passphrase."&tampilan=visible&halaman=pertama&image=false&linkQR=https://sppd.kendarikota.go.id/telaah/history/detail?telaah_id=".$telaah_idx."&xAxis=254&yAxis=".$y."&width=304&height=".$height."&jenis_response=BASE64",
	...
	function sendSignRequest4($filename, $nik, $passphrase,$telaah_idx)
	{
		$curl 	= curl_init();
		...
		curl_setopt_array($curl, array(
		CURLOPT_URL => "http://103.85.5.99/api/sign/pdf?nik=".$nik."&passphrase=".$passphrase."&tampilan=visible&halaman=pertama&image=false&linkQR=https://sppd.kendarikota.go.id/telaah/history/detail?telaah_id=".$telaah_idx."&xAxis=775&yAxis=573&width=825&height=623&jenis_response=BASE64",
		CURLOPT_RETURNTRANSFER => true,
		CURLOPT_ENCODING => "",
		CURLOPT_MAXREDIRS => 10,
		CURLOPT_TIMEOUT => 30,
	...
	function sendSignRequest2($filename, $halaman, $y, $height, $nik, $passphrase,$telaah_idx )
	{
		if($halaman==2){
			$x = "terakhir";
		} else {
		...
		CURLOPT_URL => "http://103.85.5.99/api/sign/pdf?nik=".$nik."&passphrase=".$passphrase."&tampilan=visible&halaman=".$x."&image=false&linkQR=https://sppd.kendarikota.go.id/telaah/history/detail?telaah_id=".$telaah_idx."&xAxis=220&yAxis=".$y."&width=545&height=".$height."&jenis_response=BASE64",
		CURLOPT_RETURNTRANSFER => true,
				  CURLOPT_ENCODING => "",
				  CURLOPT_MAXREDIRS => 10,
				  CURLOPT_TIMEOUT => 30,
	...
	function sendSignRequest3($filename, $halaman, $y, $height, $nik, $passphrase,$telaah_idx )
	{
		if($halaman==2){
			$x = "terakhir";
		} else {
		...
		curl_setopt_array($curl, array(
       	CURLOPT_URL => "http://103.85.5.99/api/sign/pdf?nik=".$nik."&passphrase=".$passphrase."&tampilan=visible&halaman=".$x."&image=false&linkQR=https://sppd.kendarikota.go.id/telaah/history/detail?telaah_id=".$telaah_idx."&xAxis=220&yAxis=".$y."&width=600&height=".$height."&jenis_response=BASE64",
	    	CURLOPT_RETURNTRANSFER => true,
				  CURLOPT_ENCODING => "",
				  CURLOPT_MAXREDIRS => 10,
				  CURLOPT_TIMEOUT => 30,
