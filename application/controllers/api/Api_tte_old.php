...
class Api_tte_old extends qr_Controller {
	...
	function cetak_spd($jenis_sppd, $posisi, $telaah_id, $pengikut_id, $nik, $posisi_penandatangan)
	{
		...
		if($data[0]['jenis_skpd']==5 && $data[0]['telaah_kategori']==7){
			$tanda_tangan = $this->m_pegawai->get_pegawai_nik($posisi_penandatangan,$nik, $data[0]['id_kecamatan']);
		} else {	
			$tanda_tangan = $this->m_pegawai->get_pegawai_nik($posisi_penandatangan,$nik, $data[0]['skpd_id']);
		}
	...
	function cetak_spt_dprd($telaah_id,$kategori_pelaksana,$pegawai_id,$nik, $posisi_penandatangan)
	{
        ...
        $pdf->Cell(5,5,'',0,0);
        $pdf->Cell(5,5,'a.',0,0);
        $pdf->Cell(150,5,'Perda Kota Kendari no. 7 Tahun 2019 tentang APBD Kota Kendari tahun '.date('Y').';',0,1);
			...
			if($status_nik['status_code']==1111){
						...
						foreach($data2 as $v){
							...
							## 1,6,7,9,11
								...
								// if($send_sign_request['id_signed']==""){
									// if($telaah[0]['telaah_kategori']==3){
										// $this->cetak_spd2('dprd',2,$telaah_id,$v->anggotadprd_id,$nik,$posisi);
									// } else if($telaah[0]['telaah_kategori']==8){
										// $this->cetak_spd2('walikota',2,$telaah_id,$v->pegawai_id,$nik,$posisi);
									// } else {
										// $this->cetak_spd2('opd',2,$telaah_id,$v->pegawai_id,$nik,$posisi);
									// }
				...
				if(($telaah[0]['telaah_kategori']==2 && $posisi=='walikota')
					...
					if($telaah[0]['telaah_kategori']==3){
						...
						// if($send_sign_request3['id_signed']==""){
							// $this->cetak_spt2($telaah_id,$telaah[0]['telaah_kategori'],1, $data[0]['pegawai_id'],$nik,$posisi);
						// }else {
							$sign_document3 = $this->signById($send_sign_request3['id_signed'], $this->input->post('passphrase'), $nik, $get_bearer_token['access_token']);
							$sign_document3  = json_decode($sign_document3 , TRUE);
	...
	function cekDokumen(){
		...
		//$file1 	= $_SERVER['DOCUMENT_ROOT'].'/sppd-dev/upload/doc_TTE/72d47683142b43f19c86e4113a640887.pdf';
