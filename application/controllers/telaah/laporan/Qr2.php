...
class Qr extends qr_Controller {
	...
	function cetak_spt($telaah_id, $posisi, $kategori_pelaksana, $pegawai_id, $nik, $posisi_penandatangan)
	{
		...
		$filename = 'SPT - '.$data[0]['pegawai_nama'].' - '.date("d-m-Y", strtotime($data[0]['telaah_tanggalspt'])).'.pdf';
		$path = "./upload/doc_dummy/$filename";
		$pdf->Output($path,'F');
    }
	...
	function cetak_spt_dprd($telaah_id,$kategori_pelaksana,$pegawai_id,$nik, $posisi_penandatangan)
	{
		...
		$filename = 'SPT - '.$data[0]['anggotadprd_name'].' - '.date("d-m-Y", strtotime($data[0]['telaah_tanggalspt'])).'.pdf';
		$path = "./upload/doc_dummy/$filename";
		$pdf->Output($path,'F');
    }
	...
	function generate_tte ()
	{
		...
		if($this->input->post('telaah_kategori')==3){
			...
			## Hapus PDF
			$filename = 'SPPD - '.$data[0]['anggotadprd_name'].' - '.date("d-m-Y", strtotime($data[0]['telaah_tanggalspd'])).'.pdf';
			$filename3 = 'SPT - '.$data[0]['anggotadprd_name'].' - '.date("d-m-Y", strtotime($data[0]['telaah_tanggalspt'])).'.pdf';
			...
			## Hapus PDF
			$filename = 'SPPD - '.$data[0]['pegawai_nama'].' - '.date("d-m-Y", strtotime($data[0]['telaah_tanggalspd'])).'.pdf';
			$filename3 = 'SPT - '.$data[0]['pegawai_nama'].' - '.date("d-m-Y", strtotime($data[0]['telaah_tanggalspt'])).'.pdf';
			$path_file = './upload/doc_dummy/';
			unlink($path_file.$filename);
			unlink($path_file.$filename3);
		...
		if($this->input->post('telaah_kategori')==3){
			...
			$data2 =  $this->m_pengikut->get_pengikut($this->input->post('telaah_id'));
				...
				foreach($data2 as $v){
					...
					## Hapus PDF
					if($this->input->post('telaah_kategori')==3){
						$filename2 = 'SPPD - '.$v->anggotadprd_name.' - '.date("d-m-Y", strtotime($data[0]['telaah_tanggalspd'])).'.pdf';
					} else {
						$filename2 = 'SPPD - '.$v->pegawai_nama.' - '.date("d-m-Y", strtotime($data[0]['telaah_tanggalspd'])).'.pdf';
					}
					...
					## 1,6,7,9,11
						...
						if($this->input->post('telaah_kategori')==3){
							$filename_tte_sppd = 'SPPD - '.$v->anggotadprd_name.' - '.date("d-m-Y", strtotime($data[0]['telaah_tanggalspd']));
						} else {
							$filename_tte_sppd = 'SPPD - '.$v->pegawai_nama.' - '.date("d-m-Y", strtotime($data[0]['telaah_tanggalspd']));
						}
						
						$send_sign_request   = $this->sendSignRequest($get_bearer_token['access_token'], $filename2,$set_y2,$set_height2,$nik  );
						// $send_sign_request   = $this->sendSignRequest("aaa", $filename2,$set_y2,$set_height2,$nik  );
						...
						if($send_sign_request['id_signed']==""){
							...
							$sign_document = $this->signById($get_bearer_token['access_token'],$send_sign_request['id_signed'], $this->input->post('passphrase'), $nik);
							...
							## Download Dokumen
							$download_document  = $this->downloadDokumenById($get_bearer_token['access_token'],$send_sign_request['id_signed'], $filename_tte_sppd);
		...
		if(($this->input->post('telaah_kategori')==2 && $this->input->post('posisi')=='walikota')
			...
			if($this->input->post('telaah_kategori')==3){
				$filename_tte_spt = 'SPT - '.$data[0]['anggotadprd_name'].' - '.date("d-m-Y", strtotime($data[0]['telaah_tanggalspt']));
			} else {
				$filename_tte_spt = 'SPT - '.$data[0]['pegawai_nama'].' - '.date("d-m-Y", strtotime($data[0]['telaah_tanggalspt']));
			}
			
			if($this->input->post('telaah_kategori')==3){
				$send_sign_request3   = $this->sendSignRequest3($get_bearer_token['access_token'], $filename3,$set_y3,$set_height3,$nik  );
				// $send_sign_request3   = $this->sendSignRequest3('aaa', $filename3,$set_y3,$set_height3,$nik  );
				...
				if($send_sign_request3['id_signed']==""){
					...
					$sign_document3 = $this->signById($get_bearer_token['access_token'],$send_sign_request3['id_signed'], $this->input->post('passphrase'), $nik);
				...
				$send_sign_request3   = $this->sendSignRequest2($get_bearer_token['access_token'], $filename3,$set_y3,$set_height3,$nik  );
				// $send_sign_request3   = $this->sendSignRequest2('aa', $filename3,$set_y3,$set_height3,$nik  );
				$send_sign_request3   = json_decode($send_sign_request3 , TRUE);
				if($send_sign_request3['id_signed']==""){
					...
					echo "Proses Sign : ".$sign_document3['message']."<br>";
					$download_document3  = $this->downloadDokumenById($get_bearer_token['access_token'],$send_sign_request3['id_signed'],$filename_tte_spt);
		...
		## Hanya SPD
		} else if(($this->input->post('telaah_kategori')==3 && $this->input->post('posisi')=='sekwan')
			...
			if($this->input->post('telaah_kategori')==3){
				$filename_tte_sppd = 'SPPD - '.$data[0]['anggotadprd_name'].' - '.date("d-m-Y", strtotime($data[0]['telaah_tanggalspd']));
			} else {
				$filename_tte_sppd = 'SPPD - '.$data[0]['pegawai_nama'].' - '.date("d-m-Y", strtotime($data[0]['telaah_tanggalspd']));
			}
			
			$send_sign_request   = $this->sendSignRequest($get_bearer_token['access_token'], $filename,$set_y,$set_height,$nik  );
			// $send_sign_request   = $this->sendSignRequest('aa', $filename,$set_y,$set_height,$nik  );
			$send_sign_request   = json_decode($send_sign_request , TRUE);
		...
		## 1,6,7,9,11
			...
			## Upload PDF
			$send_sign_request   = $this->sendSignRequest($get_bearer_token['access_token'], $filename,$set_y,$set_height,$nik  );
			// $send_sign_request   = $this->sendSignRequest("aaa", $filename,$set_y,$set_height,$nik  );
			$send_sign_request   = json_decode($send_sign_request , TRUE);
	...
	function getToken()
	{
		...
		## demo
		// CURLOPT_URL => "https://esign-bsre.bssn.go.id/oauth/token?client_id=12164274&client_secret=6a0p-bg14-jfrj-clbu&grant_type=client_credentials",
		CURLOPT_URL => "https://esign-bsre.bssn.go.id/oauth/token?client_id=45765303&client_secret=js4u-c6fi-58q7-gj3g&grant_type=client_credentials",
	...
	function sendSignRequest($token, $filename, $y, $height, $nik){
        $curl 	= curl_init();
        $file1 	= $_SERVER['DOCUMENT_ROOT'].'/sppd-tte/upload/doc_dummy/'.$filename;
		...
		curl_setopt_array($curl, array(
        CURLOPT_URL => "https://esign-bsre.bssn.go.id/api/v2/entity/sign/request?penandatangan=".$nik."&tampilan=visible&image=false&linkQR=http://sppd.kendarikota.go.id/&halaman=pertama&yAxis=".$y."&xAxis=105&width=545&height=".$height."",
				  CURLOPT_RETURNTRANSFER => true,
				  CURLOPT_ENCODING => "",
				  CURLOPT_MAXREDIRS => 10,
				  CURLOPT_TIMEOUT => 30,
	...
	function sendSignRequest2($token, $filename, $y, $height, $nik){
        $curl 	= curl_init();
        $file1 	= $_SERVER['DOCUMENT_ROOT'].'/sppd-tte/upload/doc_dummy/'.$filename;
        $filenames = array($file1);
        foreach ($filenames as $f){
		...
		curl_setopt_array($curl, array(
        CURLOPT_URL => "https://esign-bsre.bssn.go.id/api/v2/entity/sign/request?penandatangan=".$nik."&tampilan=visible&image=false&linkQR=http://sppd.kendarikota.go.id/&halaman=pertama&yAxis=".$y."&xAxis=220&width=545&height=".$height."",
				  CURLOPT_RETURNTRANSFER => true,
				  CURLOPT_ENCODING => "",
				  CURLOPT_MAXREDIRS => 10,
				  CURLOPT_TIMEOUT => 30,
	...
	function sendSignRequest3($token, $filename, $y, $height, $nik){
        $curl 	= curl_init();
        $file1 	= $_SERVER['DOCUMENT_ROOT'].'/sppd-tte/upload/doc_dummy/'.$filename;
        $filenames = array($file1);
        foreach ($filenames as $f){
		...
		curl_setopt_array($curl, array(
        CURLOPT_URL => "https://esign-bsre.bssn.go.id/api/v2/entity/sign/request?penandatangan=".$nik."&tampilan=visible&image=false&linkQR=http://sppd.kendarikota.go.id/&halaman=pertama&yAxis=".$y."&xAxis=220&width=600&height=".$height."",
				  CURLOPT_RETURNTRANSFER => true,
				  CURLOPT_ENCODING => "",
				  CURLOPT_MAXREDIRS => 10,
				  CURLOPT_TIMEOUT => 30,
    ...
    function signById($token, $id, $passphrase, $nik){
        $curl = curl_init();

        curl_setopt_array($curl, array(
        CURLOPT_URL => "https://esign-bsre.bssn.go.id/api/v2/entity/sign/".$id."?passphrase=$passphrase&approved_info=ok&nik=$nik",
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_SSL_VERIFYPEER => false,
        CURLOPT_SSL_VERIFYHOST => false,
        CURLOPT_ENCODING => "",
    ...
    function downloadDokumenById($token, $id_signed, $filename){
        //$id_dokumen = $request['id_dokumen'];
		//$path = './upload/doc_TTE/'.$id_signed.'.pdf';
		$path = './upload/doc_TTE/'.$filename.'.pdf';
        $curl = curl_init();

        curl_setopt_array($curl, array(
        CURLOPT_URL => "https://esign-bsre.bssn.go.id/api/v2/entity/sign/download/".$id_signed,
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_SSL_VERIFYPEER => false,
        CURLOPT_SSL_VERIFYHOST => false,
        CURLOPT_ENCODING => "",
        ...
        // header('Content-Disposition: attachment; filename="'.$id_signed.'".pdf"');
	...
	function doCekStatus(){
        ...
        curl_setopt_array($curl, array(
        CURLOPT_URL => "https://esign-bsre.bssn.go.id/api/v2/entity/status/7471086305630001",
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => "",
        CURLOPT_SSL_VERIFYPEER => false,
        CURLOPT_SSL_VERIFYHOST => false,
        ...
        CURLOPT_POSTFIELDS => "",
        CURLOPT_HTTPHEADER => array(
            "Authorization: Bearer b995faca-8767-4767-bc6b-0849366fcad0",
            "cache-control: no-cache"
        ),
        ));
