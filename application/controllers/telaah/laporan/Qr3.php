...
class Qr3 extends qr_Controller {
	...
	{
		$telaah_id = $this->input->get('telaah_id');
	...
	function cetak_spd($jenis_sppd, $posisi, $telaah_id, $pengikut_id, $nik, $posisi_penandatangan)
	{
		...
		$pdf->Cell(45,4,': KOTA KENDARI','TR',1);
		...
		if(($data[0]['telaah_kategori']==1 && $data[0]['jenis_skpd']!=7) || ($data[0]['telaah_kategori']==2) || ($data[0]['jenis_skpd']==3)){
			if($data[0]['telaah_domainperjalanan']==1 || $data[0]['telaah_domainperjalanan']==2 ){
				...
				$align = array('','','','','','');
				$caption = array('I.','Tiba Di',': '.$data[0]['kabupaten_kota'],'','Berangkat dari',': ');
				$pdf->Row($caption, $border, $align);
			} else {
				$pdf->Cell(5,4,'II.','LT',0);
				$pdf->Cell(30,4,'Tiba Di','T',0);
				$pdf->Cell(45,4,': '.$data[0]['kabupaten_kota'],'T',0);
				$pdf->Cell(5,4,'','LT',0);
				$pdf->Cell(30,4,'Berangkat dari','T',0);
				$pdf->Cell(45,4,': ','TR',1);
			}
		...
		if(($data[0]['telaah_kategori']==1 && $data[0]['jenis_skpd']!=7) || ($data[0]['telaah_kategori']==2)){
			if($data[0]['telaah_domainperjalanan']==1 || $data[0]['telaah_domainperjalanan']==2 ){
				...
				$align = array('','','','');
				if($data[0]['skpd_id']==38){
					$caption = array('VII.','Tiba Di',': KOTA KENDARI','Telah diperiksa dengan keterangan bahwa');
				}else {
					$caption = array('VI','Tiba Di',': KOTA KENDARI','Telah diperiksa dengan keterangan bahwa');
				}
				$pdf->Row($caption, $border, $align);
			} else {
				if($data[0]['skpd_id']==38){
				...
				$pdf->Cell(30,4,'Tiba Di','T',0);
				$pdf->Cell(45,4,': KOTA KENDARI','RT',0);
	...
	function cetak_spd2($jenis_sppd, $posisi, $telaah_id, $pengikut_id, $nik, $posisi_penandatangan)
	{
		...
		QRcode::png(base_url().'telaah/laporan/qr3/sppd?telaah_id='.$telaah_id,"test.png");
		// $pdf->Image("test.png", 185, 10, 20, 20, "png");
		...
		$y=$pdf->GetY();
		$pdf->MultiCell(10,0,$pdf->Image("test.png", 105, $y, 20, 20, "png"),0,0);
		...
		$y=$pdf->GetY();
		$pdf->MultiCell(10,0,$pdf->Image("test.png", 137, $y+5, 20, 20, "png"),0,0);
	...
	function cetak_spt2($telaah_id, $posisi, $kategori_pelaksana, $pegawai_id, $nik, $posisi_penandatangan)
	{
		...
		$y=$pdf->GetY();
		$pdf->MultiCell(10,0,$pdf->Image("test.png", 125, $y, 20, 20, "png"),0,0);		
		$pdf->Cell(10,2,'',0,1);	
	...
	function cetak_spt_dprd2($telaah_id,$kategori_pelaksana,$pegawai_id,$nik, $posisi_penandatangan)
	{
		...
		$y=$pdf->GetY();
		$pdf->MultiCell(10,0,$pdf->Image("test.png", 135, $y, 20, 20, "png"),0,0);		
		$pdf->Cell(10,20,'',0,1);
        $pdf->Cell(80,5,'',0,0);
        $pdf->Cell(80,5,$tanda_tangan[0]['pegawai_nama'],0,1,'C');
