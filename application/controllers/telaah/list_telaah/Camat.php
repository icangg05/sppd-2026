...
class Camat extends public_Controller {
	...
	{
		$config = array ();
		$config ["base_url"] = base_url () . "list_telaah/camat/index";
		...
		$this->data['camat'] = $this->m_camat->data($config ["per_page"], $page, $this->ion_auth->user()->row()->skpd_id);
	...
	public function search()
	{
		...
		$config = array ();
		$config ["base_url"] = base_url () . "list_telaah/camat/search/".$query."/".$column;
		...
		$this->data['links'] = $this->pagination->create_links ();
		$this->data['camat'] = $this->m_camat->data_search($column,$query,$config ["per_page"], $page,$this->ion_auth->user()->row()->skpd_id);
		$this->render('list_telaah/camat/content');
	}
	//View Data Result
	...
	{
		$this->data['pegawai'] = $this->m_camat->pegawai($this->ion_auth->user()->row()->skpd_id);
		$this->data['anggaran'] = $this->m_camat->anggaran($this->ion_auth->user()->row()->skpd_id);
		$this->data['rekening'] = $this->m_camat->rekening($this->ion_auth->user()->row()->skpd_id);
		$this->data['provinsi'] = $this->m_camat->get_provinsi();
		$this->data['kabupaten'] = $this->m_camat->get_kabupaten();
		$this->data['posisi_walikota'] = $this->m_camat->posisi_walikota($this->ion_auth->user()->row()->skpd_id);
		$this->render('list_telaah/camat/insert');
	}
	...
	public function detail()
	{
		...
		if(!isset($this->data['entry'][0]) || $this->data['entry'][0] == ""){
			...
			$this->data['timeline'] =  $this->m_camat->getTimeline($telaah_id);
	...
	public function staf_detail()
	{
		...
		if(!isset($this->data['entry'][0]) || $this->data['entry'][0] == ""){
			...
			$this->data['pengikut'] =  $this->m_pengikut->data($telaah_id);
			$this->data['timeline'] =  $this->m_camat->getTimeline($telaah_id);
			$this->render('list_telaah/camat/staf_detail');
		}
	}
	...
	public function delete_telaah(){
		...
		for($i=0;$i<count($arrayTable);$i++){
			$this->m_camat->delete_telaah($this->input->post('telaah_id'), $arrayTable[$i]);
		}
