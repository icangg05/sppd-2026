...
class Dprd extends public_Controller {
	...
	{
		$config = array ();
		$config ["base_url"] = base_url () . "list_telaah/dprd/index";
		...
		$this->data['dprd'] = $this->m_dprd->data($config ["per_page"], $page);
	...
	public function search()
	{
		...
		$config = array ();
		$config ["base_url"] = base_url () . "dprd/search/".$query."/".$column;
		...
		$this->data['links'] = $this->pagination->create_links ();
		$this->data['dprd'] = $this->m_dprd->data_search($column,$query,$config ["per_page"], $page);
 		$this->render('list_telaah/dprd/content');
	}
	//View Data Result
    ...
    {
		$this->data['anggaran'] = $this->m_dprd->anggaran($this->ion_auth->user()->row()->skpd_id);
		$this->data['rekening'] = $this->m_dprd->rekening($this->ion_auth->user()->row()->skpd_id);
		$this->data['anggota'] = $this->m_dprd->anggota();
		$this->data['provinsi'] = $this->m_dprd->get_provinsi();
		$this->data['kabupaten'] = $this->m_dprd->get_kabupaten();
		$this->data['posisi_kadprd'] = $this->m_dprd->posisi_kadprd();
        $this->render('list_telaah/dprd/insert');
	}
	...
	public function create()
	{
		...
		{	
			...
			if(!$this->upload->do_upload()){
				...
				$this->m_dprd->create($data);
				
				$data2['anggotadprd_id'] = $this->input->post('telaah_pelaksana');
				$data2['status'] = 1;
				$this->m_anggota->update($data2);
				
				$last = $this->m_dprd->getLast();
						foreach ($last as $l) {
								$last_id = $l->telaah_id;
						}
				...
				for($i=0;$i<$jumlah;$i++) {
					...
					$data4['anggotadprd_id'] = $this->input->post('telaah_pengikut')[$i];
					$data4['status'] = 1;
					$this->m_anggota->update($data4);
				}		
	...
	public function detail()
	{
		...
		if(!isset($this->data['entry'][0]) || $this->data['entry'][0] == ""){
			...
			$this->data['timeline'] =  $this->m_dprd->getTimeline($telaah_id);
			$this->render('list_telaah/dprd/detail');
		}
	}
	
	public function get(){
		$data ['get_kabkot']=$this->m_dprd->get_kabkot($this->uri->segment(4));
	...
	public function cek_timeline(){
		...
		$this->render('list_telaah/dprd/cek_timeline');
	}
	...
	public function delete_telaah(){
		...
		for($i=0;$i<count($arrayTable);$i++){
			$this->m_dprd->delete_telaah($this->input->post('telaah_id'), $arrayTable[$i]);
		}
