...
class Kadis extends public_Controller
{
	...
	{
		$config = array();
		$config["base_url"] = base_url() . "list_telaah/kadis/index";
		...
		$this->data['kadis'] = $this->m_kadis->data($config["per_page"], $page, $this->ion_auth->user()->row()->skpd_id);
	...
	public function search()
	{
		...
		$config = array();
		$config["base_url"] = base_url() . "list_telaah/kadis/search/" . $query . "/" . $column;
		...
		$this->data['links'] = $this->pagination->create_links();
		$this->data['kadis'] = $this->m_kadis->data_search($column, $query, $config["per_page"], $page, $this->ion_auth->user()->row()->skpd_id);
		$this->render('list_telaah/kadis/content');
	}
	...
	public function create_view()
	{
		...
		$this->data['pegawai'] = $this->m_kadis->pegawai($this->ion_auth->user()->row()->skpd_id);
		$this->data['anggaran'] = $this->m_kadis->anggaran($this->ion_auth->user()->row()->skpd_id);
		$this->data['rekening'] = $this->m_kadis->rekening($this->ion_auth->user()->row()->skpd_id);
		$this->data['provinsi'] = $this->m_kadis->get_provinsi();
		$this->data['kabupaten'] = $this->m_kadis->get_kabupaten();
		$this->data['posisi_walikota'] = $this->m_kadis->posisi_walikota();
		$this->render('list_telaah/kadis/insert');
	}
	...
	public function detail()
	{
		...
		if (!isset($this->data['entry'][0]) || $this->data['entry'][0] == "") {
			...
			$this->data['timeline'] =  $this->m_kadis->getTimeline2($telaah_id);
	...
	public function get()
	{
		$data['get_kabkot'] = $this->m_kadis->get_kabkot($this->uri->segment(4));
	...
	public function cek_timeline()
	{
		...
		$this->render('list_telaah/kadis/cek_timeline');
	}
