...
class Kapus extends public_Controller {
	function __construct(){
		parent::__construct();
		$this->load->model('m_kapus');
	...
	public function index(){
		$config = array ();
		$config ["base_url"] = base_url () . "list_telaah/kapus/index";
		...
		$this->data['kapus'] = $this->m_kapus->data($config ["per_page"], $page, $this->ion_auth->user()->row()->skpd_id);
	...
	public function search(){
		...
		$config = array ();
		$config ["base_url"] = base_url () . "list_telaah/kapus/search/".$query."/".$column;
		...
		$this->data['links'] = $this->pagination->create_links ();
		$this->data['kapus'] = $this->m_kapus->data_search($column,$query,$config ["per_page"], $page, $this->ion_auth->user()->row()->skpd_id);
		$this->render('list_telaah/kapus/content');
	}
	...
	public function create_view()
	{
		...
		$this->data['anggaran'] = $this->m_kapus->anggaran($this->ion_auth->user()->row()->skpd_id);
		$this->data['rekening'] = $this->m_kapus->rekening($this->ion_auth->user()->row()->skpd_id);
		$this->data['provinsi'] = $this->m_kapus->get_provinsi();
		$this->data['kabupaten'] = $this->m_kapus->get_kabupaten();
		$this->data['posisi_walikota'] = $this->m_kapus->posisi_walikota();
		$this->render('list_telaah/kapus/insert');
	}
	...
	public function create()
	{
		...
		{	
			...
			if(!$this->upload->do_upload()){
				...
				} else if($data['telaah_domainperjalanan']==4){
					$data['telaah_provinsitujuan'] = 74;
					$data['telaah_kotatujuan'] = 7471;
				}
	...
	public function get(){
		$data ['get_kabkot']=$this->m_kapus->get_kabkot($this->uri->segment(4));
	...
	public function cek_timeline(){
		...
		$this->render('list_telaah/kapus/cek_timeline');
	}
	...
	public function delete_telaah(){
		...
		for($i=0;$i<count($arrayTable);$i++){
			$this->m_kapus->delete_telaah($this->input->post('telaah_id'), $arrayTable[$i]);
		}
