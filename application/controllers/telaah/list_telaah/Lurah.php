	...
	class Lurah extends public_Controller {
		function __construct()
		{
			parent::__construct();
			$this->load->model('m_lurah');
	...
	//View All Data
		...
		{
			$config = array ();
			$config ["base_url"] = base_url () . "list_telaah/lurah/index";
			...
			$this->data['lurah'] = $this->m_lurah->data($config ["per_page"], $page, $this->ion_auth->user()->row()->skpd_id);
	...
	//View Data Search
		public function search()
		{
			...
			$config = array ();
			$config ["base_url"] = base_url () . "list_telaah/lurah/search/".$query."/".$column;
			...
			$this->data['links'] = $this->pagination->create_links ();
			$this->data['lurah'] = $this->m_lurah->data_search($column,$query,$config ["per_page"], $page,$this->ion_auth->user()->row()->skpd_id);
			$this->render('list_telaah/lurah/content');
		}
	//View Data Result
	...
	//View Create Data
		...
		{
			$this->data['pegawai'] = $this->m_lurah->pegawai($this->ion_auth->user()->row()->skpd_id);
			$this->data['anggaran'] = $this->m_lurah->anggaran($this->ion_auth->user()->row()->skpd_id);
			$this->data['rekening'] = $this->m_lurah->rekening($this->ion_auth->user()->row()->skpd_id);
			$this->data['provinsi'] = $this->m_lurah->get_provinsi();
			$this->data['kabupaten'] = $this->m_lurah->get_kabupaten();
			$this->data['posisi_walikota'] = $this->m_lurah->posisi_walikota($this->ion_auth->user()->row()->skpd_id);
			$this->render('list_telaah/lurah/insert');
		}
	...
	//View Detail Data
		public function detail()
		{
			...
			if(!isset($this->data['entry'][0]) || $this->data['entry'][0] == ""){
				...
				$this->data['timeline'] =  $this->m_lurah->getTimeline($telaah_id);
	...
	//View Detail Data
		public function staf_detail()
		{
			...
			if(!isset($this->data['entry'][0]) || $this->data['entry'][0] == ""){
				...
				$this->data['pengikut'] =  $this->m_pengikut->data($telaah_id);
				$this->data['timeline'] =  $this->m_lurah->getTimeline($telaah_id);
				$this->render('list_telaah/lurah/staf_detail');
			}
		}
	...
	public function delete_telaah(){
		...
		for($i=0;$i<count($arrayTable);$i++){
			$this->m_lurah->delete_telaah($this->input->post('telaah_id'), $arrayTable[$i]);
		}
