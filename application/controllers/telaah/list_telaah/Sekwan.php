...
class Sekwan extends public_Controller {
	function __construct(){
		parent::__construct();
		$this->load->model('m_sekwan');
	...
	//View All Data Permohonan SPPD
	public function index(){
		$config = array ();
		$config ["base_url"] = base_url () . "list_telaah/sekwan/index";
		...
		$this->data['sekwan'] = $this->m_sekwan->data($config ["per_page"], $page, $this->ion_auth->user()->row()->skpd_id);
	...
	public function search(){
		...
		$config = array ();
		$config ["base_url"] = base_url () . "list_telaah/sekwan/search/".$query."/".$column;
		...
		$this->data['links'] = $this->pagination->create_links ();
		$this->data['sekwan'] = $this->m_sekwan->data_search($column,$query,$config ["per_page"], $page, $this->ion_auth->user()->row()->skpd_id);
		$this->render('list_telaah/sekwan/content');
	}
	...
	public function create_view()
	{
		...
		$this->data['pegawai'] = $this->m_sekwan->pegawai($this->ion_auth->user()->row()->skpd_id);
		$this->data['anggaran'] = $this->m_sekwan->anggaran($this->ion_auth->user()->row()->skpd_id);
		$this->data['rekening'] = $this->m_sekwan->rekening($this->ion_auth->user()->row()->skpd_id);
		$this->data['provinsi'] = $this->m_sekwan->get_provinsi();
		$this->data['kabupaten'] = $this->m_sekwan->get_kabupaten();
		$this->data['posisi_walikota'] = $this->m_sekwan->posisi_walikota();
		$this->render('list_telaah/sekwan/insert');
	}
	...
	public function get(){
		$data ['get_kabkot']=$this->m_sekwan->get_kabkot($this->uri->segment(4));
	...
	public function cek_timeline(){
		$telaah_id = $this->encrypt->decode(base64_decode($this->input->get('telaah_id')), $this->session->userdata('encrypt_key'));
		$this->data['timeline']	=$this->m_sekwan->getTimeline2($telaah_id);
		$this->data['data_telaah']		=$this->m_sekwan->get($telaah_id);
		$this->render('list_telaah/sekwan/cek_timeline');
	}
	...
	public function delete_telaah(){
		...
		for($i=0;$i<count($arrayTable);$i++){
			$this->m_sekwan->delete_telaah($this->input->post('telaah_id'), $arrayTable[$i]);
		}
