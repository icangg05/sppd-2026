<?php
defined('BASEPATH') OR exit('number direct script access allowed');
class Staff_lurah extends public_Controller {
	function __construct()
	{
		parent::__construct();
		$this->load->model('m_staff_lurah');
...
//View All Data
	...
	{
		$config = array ();
		$config ["base_url"] = base_url () . "staff_lurah/index";
		...
		$config ['full_tag_open'] = '<ul class="pagination pagination-sm number-margin pull-right">';
		...
		$this->data['staff_lurah'] = $this->m_staff_lurah->data($config ["per_page"], $page, $this->ion_auth->user()->row()->skpd_id);
...
//View Data Search
	public function search()
	{
		...
		$config = array ();
		$config ["base_url"] = base_url () . "staff_lurah/search/".$query."/".$column;
		...
		$this->data['links'] = $this->pagination->create_links ();
		$this->data['staff_lurah'] = $this->m_staff_lurah->data_search($column,$query,$config ["per_page"], $page, $this->ion_auth->user()->row()->skpd_id);
		$this->render('list_telaah/staff_lurah/content');
	}
//View Data Result
...
//View Create Data
	...
	{
		$this->data['pegawai'] = $this->m_staff_lurah->pegawai($this->ion_auth->user()->row()->skpd_id);
		$this->data['anggaran'] = $this->m_staff_lurah->anggaran($this->ion_auth->user()->row()->skpd_id);
		$this->data['rekening'] = $this->m_staff_lurah->rekening($this->ion_auth->user()->row()->skpd_id);
		$this->data['provinsi'] = $this->m_staff_lurah->get_provinsi();
		$this->data['kabupaten'] = $this->m_staff_lurah->get_kabupaten();
		$this->data['posisi_kaopd'] = $this->m_staff_lurah->posisi_kaopd($this->ion_auth->user()->row()->skpd_id);
		$this->render('list_telaah/staff_lurah/insert');
	}
	...
	public function create()
	{
		...
		{	
			...
			if(!$this->upload->do_upload()){
				...
				$this->session->set_flashdata('numbertif','Data Telaah Staf Di Simpan !');
...
//View Detail Data
	...
	public function get(){
		$data ['get_kabkot']=$this->m_staff_lurah->get_kabkot($this->uri->segment(4));
...
//View Laporan
	...
	public function delete_telaah(){
		...
		for($i=0;$i<count($arrayTable);$i++){
			$this->m_staff_lurah->delete_telaah($this->input->post('telaah_id'), $arrayTable[$i]);
		}

		
		$this->session->set_flashdata('numbertif','Data Telaah Staf Di Hapus !');
