...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, skpd_nama, jenis_skpd')
						  ->where($this->identity_column, $identity)
						  ->limit(1)
						  ->order_by('a.id', 'desc')
						  ->get('users a, table_skpd b');
	...
	public function user($id = NULL)
	{
		...
		return $this->db->query("select *, users.id as user_id from users
								left join table_skpd ON users.skpd_id = table_skpd.skpd_id
								where users.id = '$id' ");
	}
	...
	public function wip()
	{
		...
		$this->response = $this->db->get($this->tables['table_wip']);

		return $this;
	}
	
	public function create_wip($additional_data)
	{
		/*
		// bail if the group name was not passed
		if(!$group_name)
		...
		*/
		...
		// insert the new group
		$this->db->insert($this->tables['table_wip'], $additional_data);
		//$group_id = $this->db->insert_id($this->tables['groups'] . '_id_seq');
